...
ops.readTiffHeader = getOr(ops,'readTiffHeader',0);
...
%         ffile = dir(fullfile(ops.RootDir, ops.SubDirs{j}, '*.raw'));
...
%         end
%         
        n = fullfile(ops.RootDir,[ops.date '_' ops.mouse_name '_t-' num2str(str2num(ops.SubDirs{j}),'%03d') '.bin']);
        ops.fsroot{j} = dir(n);
        ops.fsroot{j}(1).name = n;
    end
    
    if isfield(db, 'expred') && ~isempty(db.expred) && ...
            (~isfield(db, 'nchannels_red') || isempty(db.nchannels_red))
        ffile = dir(fullfile(ops.RootDir, num2str(db.expred), '*.raw'));
        fname = struct2cell(ffile);
        fname = fname(1,:)';
        [~,index] = sort_nat(fname);
